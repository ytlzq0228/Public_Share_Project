...
import sys
import hashlib
sys.path.append(r"C:\VPNapi\ApiSite\GoogleAPI")
...
def run_bat():
	try:
		...
		f = open("C:\VPNapi\ApiSite\log\log.txt",'a')
		f.writelines('\n'+now)
		f.flush()
		f.close()
		run_result = os.system("test.bat >> C:\VPNapi\ApiSite\log\log.txt")
...
def get_data(request):
	...
	if request.method == "POST":	# 定义post接口
		...
		try:
			...
			if SECRET:
				save_log("GET SECRET:%s"%hashlib.sha256(SECRET.encode()).hexdigest())
